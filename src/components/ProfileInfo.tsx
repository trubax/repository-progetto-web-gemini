  ...
  return (
    <div className="space-y-6 p-4">
      ...
      <div className="space-y-2">
        ...
        >
          <span>{showDemographics ? 'Nascondi' : 'Mostra'} informazioni demografiche</span>
        </button>
        ...
        {showDemographics && (
          <div className="space-y-2 p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            {profile.birthYear && (
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Anno di nascita:</span>
                <span className="text-sm text-gray-700 dark:text-gray-300">{profile.birthYear}</span>
              </div>
            )}
            
            {profile.country && (
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Paese:</span>
                <span className="text-sm text-gray-700 dark:text-gray-300">
                  {profile.country === 'IT' ? 'Italia' :
                   profile.country === 'US' ? 'Stati Uniti' :
                   profile.country === 'GB' ? 'Regno Unito' :
                   profile.country === 'DE' ? 'Germania' :
                   profile.country === 'FR' ? 'Francia' :
                   profile.country === 'ES' ? 'Spagna' :
                   profile.country === 'PT' ? 'Portogallo' :
                   profile.country === 'NL' ? 'Paesi Bassi' :
                   profile.country === 'BE' ? 'Belgio' :
                   profile.country === 'CH' ? 'Svizzera' :
                   profile.country}
                </span>
              </div>
            )}
            ...
            {profile.platform && (
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Piattaforma:</span>
                <span className="text-sm text-gray-700 dark:text-gray-300">{profile.platform}</span>
              </div>
            )}
          </div>
        )}
