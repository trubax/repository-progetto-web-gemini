...
export function useCreateChat() {
  ...
  const createChat = async (participantId: string): Promise<ChatPreview | null> => {
    ...
    try {
      ...
      // Crea oggetto con i dati dei partecipanti
      const participantsData = {
        ...
        [participantId]: {
          displayName: participantData.displayName || 'Utente',
      ...
      return {
        id: chatRef.id,
        name: participantData.displayName || 'Utente',
        photoURL: participantData.photoURL || '',
        lastMessage: '',
        timestamp: 'Nuovo',
        unread: 0,
  ...
  return { createChatWithMessage, createGroupWithMessage, createChat, loading };
}