...
export class PermissionsService {
  ...
  async checkAndRequestPermissions(type: 'audio' | 'video' | 'both'): Promise<{
    ...
    try {
      ...
      // Se i permessi non sono stati concessi, richiedi l'accesso
      const { hasPermission, error } = await this.checkMediaPermissions(type);

      if (!hasPermission) {
        return {
          granted: false,
          error: error || 'Permessi microfono non concessi. Verifica le impostazioni del browser.'
        };
      }
      ...
      return {
        granted: false,
        error: 'Errore durante la verifica dei permessi. Verifica le impostazioni del browser.'
      };
    }
  }
}